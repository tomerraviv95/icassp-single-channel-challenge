...
if __name__ == "__main__":
    ...
    test_meta_data = np.load(
        os.path.join('dataset', f'{TESTSET_IDENTIFIER}_testmixture_{SOI_TYPE}_{interference}_metadata.npy'))
    ...
    sig_est = torch.concat(total_pred_x_gt, dim=0).numpy()
    bit_est = torch.concat(total_pred_bits_gt, dim=0).numpy()
    np.save(
